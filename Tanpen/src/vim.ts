...
type Unit = 'character' | 'word' | 'sentence'
...
class Vim {
    ...
    moveByCharacter(direction: Direction, times: number=1) {
        while (times-- > 0) {
            ...
            else this.selection.collapseToEnd()
            this.selection.modify('move', direction, 'character')
            this.selection.modify('extend', 'backward', 'character')

            if (this.textSelected === '\n')
                if (this.selection.focusOffset === 1) ++times  // `\n` at the end of line will be ignored
                else this.selection.collapseToStart()  // Make `\n` block displayed as hairline
    ...
    moveByWord(direction: Direction, times: number=1) {
        ...
        while (this.textSelected.match(/[\s\p{Punctuation}]/u)) {
            ...
            if (this.selection.focusOffset === lastOffset) break
            else lastOffset = this.selection.focusOffset
        }
    }
    ...
    constructor(element: HTMLElement, mode: Mode='insert') {
